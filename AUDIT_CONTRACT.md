...
- `audit-verify --expect-pack-fingerprint <64hex>` / `audit-handoff-verify --expect-pack-fingerprint <64hex>` enforce external fingerprint pinning to prevent PACK substitution.
...
- `--profile default` is diagnostics-only (non-audit acceptance): trusted signature/attestation requirements are not enforced.
